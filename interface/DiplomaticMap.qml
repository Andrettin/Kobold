...
Flickable {
	id: diplomatic_map
	contentWidth: kobold.map.diplomatic_map_image_size.width * scale_factor
	contentHeight: kobold.map.diplomatic_map_image_size.height * scale_factor
	...
	readonly property var reference_country: selected_country ? selected_country : (kobold.game.player_country ? kobold.game.player_country : null)
	...
	*/
	...
	Image {
		...
		source: kobold.game.running ? "image://diplomatic_map/exploration" : "image://empty/"
		cache: false
	...
	Repeater {
		...
		Image {
			id: consulate_icon
			x: other_country_capital ? (other_country_capital.game_data.tile_pos.x * kobold.map.diplomatic_map_tile_pixel_size * scale_factor - width / 2) : 0
			y: other_country_capital ? (other_country_capital.game_data.tile_pos.y * kobold.map.diplomatic_map_tile_pixel_size * scale_factor - height / 2) : 0
	...
	function center_on_tile_pos(tile_x, tile_y) {
		var pixel_x = Math.round(tile_x * kobold.map.diplomatic_map_tile_pixel_size * scale_factor - diplomatic_map.width / 2)
		var pixel_y = Math.round(tile_y * kobold.map.diplomatic_map_tile_pixel_size * scale_factor - diplomatic_map.height / 2)

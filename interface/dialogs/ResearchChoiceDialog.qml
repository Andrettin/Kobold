	...
	Column {
		...
		Repeater {
			...
			TextButton {
				...
				readonly property string effects_string: technology.get_effects_string(kobold.game.player_country)
				
				onClicked: {
					research_choice_dialog.close()
					if (free_technology) {
						kobold.game.player_country.game_data.gain_free_technology(technology)
					} else {
						kobold.game.player_country.game_data.current_research = technology
					}
				}
			}
		}

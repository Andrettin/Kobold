...
Item {
	...
	Image {
		...
		source: "image://icon/" + (visible && is_on_water ? (
			(province.game_data.military_unit_category_counts.length > 0 && province.game_data.get_country_military_unit_category_counts(kobold.game.player_country).length > 0) ?
				province.game_data.get_country_military_unit_icon(kobold.game.player_country).identifier
	...
	*/
	...
	MouseArea {
		...
		onReleased: {
			var explored = kobold.game.player_country.game_data.is_tile_explored(tile_pos)
			...
			if (kobold.selected_military_units.length > 0) {
				kobold.move_selected_military_units_to(tile_pos)
				selected_civilian_unit = null
				selected_site = null
				selected_province = null
				selected_garrison = false
	...
	MouseArea {
		...
		hoverEnabled: true
		enabled: garrison_icon.visible && province !== null && ((province.water_zone && province.game_data.military_unit_category_counts.length > 0 && province.game_data.get_country_military_unit_category_counts(kobold.game.player_country).length > 0) || (province.game_data.owner !== null && (province.game_data.owner === kobold.game.player_country || province.game_data.owner.game_data.is_any_vassal_of(kobold.game.player_country))))
		visible: enabled

...
{
	if (parent.isValid()) {
		return static_cast<int>(reinterpret_cast<const kobold::scenario *>(parent.constInternalPointer())->get_child_scenarios().size());
	} else {
...
QModelIndex scenario_model::parent(const QModelIndex &index) const
{
	...
	const kobold::scenario *grandparent_scenario = parent_scenario->get_parent_scenario();

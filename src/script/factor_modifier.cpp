...
void factor_modifier<scope_type>::process_gsml_property(const gsml_property &property)
{
	...
	if (key == "factor") {
		...
		std::unique_ptr<const condition_base<scope_type, read_only_context>> condition = kobold::condition<scope_type>::from_gsml_property(property);
...
{
	std::unique_ptr<const condition_base<scope_type, read_only_context>> condition = kobold::condition<scope_type>::from_gsml_scope(scope);
	this->conditions->add_condition(std::move(condition));
}

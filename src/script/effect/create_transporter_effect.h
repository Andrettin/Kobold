...
public:
	...
	virtual void do_assignment_effect(const country *scope, context &ctx) const override
	{
		...
		auto transporter = make_qunique<kobold::transporter>(type, scope, culture, scope->get_game_data()->get_religion(), culture->get_default_phenotype(), scope->get_game_data()->get_capital());

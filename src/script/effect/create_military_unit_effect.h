...
public:
	...
	virtual void do_assignment_effect(const country *scope, context &ctx) const override
	{
		...
		auto military_unit = make_qunique<kobold::military_unit>(type, scope, culture, scope->get_game_data()->get_religion(), culture->get_default_phenotype(), scope->get_game_data()->get_capital());

...
bool terrain_geodata_map_compare::operator()(const terrain_geodata_key &terrain_variant, const terrain_geodata_key  &other_terrain_variant) const
{
	...
	if (std::holds_alternative<const kobold::terrain_feature *>(terrain_variant)) {
		terrain_feature = std::get<const kobold::terrain_feature *>(terrain_variant);
	...
	const kobold::terrain_feature *other_terrain_feature = nullptr;
	if (std::holds_alternative<const kobold::terrain_feature *>(other_terrain_variant)) {
		other_terrain_feature = std::get<const kobold::terrain_feature *>(other_terrain_variant);
